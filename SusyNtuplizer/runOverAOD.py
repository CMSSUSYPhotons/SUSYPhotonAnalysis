...
# IsoDeposit - runs PU identificaton over particleFlow, then calculates isodeposits around given particles using the NoPU collection
# Transient products:
#  PFCandidateCollection:pfPileUpIso = PU particles
#  PFCandidateCollection:pfNoPileUpIso = non-PU particles identified using PU particles (there is no "non-PU" tagging)
#  PFCandidateCollection:pfAll(NeutralHadrons|ChargedHadrons|Photons|ChargedParticles|NeutralHadronsAndPhotons) = non-PU particles selected based on pdgId
#  PFCandidateCollection:pfPileUpAllChargedParticles = PU particles selected based on pdgId
#  IsoDepositMap:(el|ph)PFIsoDeposit(Charged|ChargedAll|Neutral|Gamma|PU) = mapping between particle and IsoDeposit (isolation values as a function of dR)
#  ValueMap<double>:(el|ph)PFIsoValue(Charged|ChargedAll|Neutral|Gamma|PU)0[34](PF|NoPF)Id = isolation values obtained from isodeposit. PF and NoPF are identical
# The whole sequence includes some unnecessary processings; can we define our own?
