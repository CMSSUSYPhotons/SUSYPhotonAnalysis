...
JetCorrectorParameters const & JetCorrectorParametersCollection::operator[]( key_type k ) const {
  ...
  //@@ dwjang // //  throw cms::Exception("InvalidInput") << " cannot find key " << static_cast<int>(k) << std::endl;
}
...
JetCorrectorParametersCollection::findKey( std::string const & label ) const {
  ...
  //@@ dwjang // // Didn't find default corrections, throw exception
  //@@ dwjang // //  throw cms::Exception("InvalidInput") << " Cannot find label " << label << std::endl;
...
//@@ dwjang // //#include "FWCore/Utilities/interface/typelookup.h"
 
//@@ dwjang // //TYPELOOKUP_DATA_REG(JetCorrectorParameters);
//@@ dwjang // //TYPELOOKUP_DATA_REG(JetCorrectorParametersCollection);
