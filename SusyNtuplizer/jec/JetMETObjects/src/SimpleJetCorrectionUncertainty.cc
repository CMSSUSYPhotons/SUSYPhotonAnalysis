...
//@@ dwjang // //   if (bin<0) 
//@@ dwjang // //     throw cms::Exception("SimpleJetCorrectionUncertainty")<<" bin variables out of range";
...
{
//@@ dwjang // //   if (fBin >= mParameters->size()) 
//@@ dwjang // //     throw cms::Exception("SimpleJetCorrectionUncertainty")<<" wrong bin: "<<fBin<<": only "<<mParameters->size()<<" are available";
  const std::vector<float>& p = mParameters->record(fBin).parameters();
//@@ dwjang // //   if ((p.size() % 3) != 0)
//@@ dwjang // //     throw cms::Exception ("SimpleJetCorrectionUncertainty")<<"wrong # of parameters: multiple of 3 expected, "<<p.size()<< " got";
  ...
  if (fX[0] == fX[1])
    {
      if (fY[0] == fY[1])
        r = fY[0];
//@@ dwjang // //       else
//@@ dwjang // //         throw cms::Exception("SimpleJetCorrectionUncertainty")<<" interpolation error";
    } 
  else   
    {
